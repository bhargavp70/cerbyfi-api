...
class ScoreDB:
    def __init__(self, db_path: str, ttl_seconds: int):
        ...
        with self._lock:
            ...
            for col_sql in [
                ...
                "ALTER TABLE portfolio_holdings ADD COLUMN max_alloc REAL",
            ]:
                try:
    ...
    def get_holdings(self, portfolio_id: str) -> list:
        with self._lock:
            rows = self._conn.execute(
                ...
                "shares, avg_cost, purchase_date, max_alloc "
    ...
    def get_ai_usage(self, user_id: str, month: str) -> int:
        """Return number of AI reports generated by user in the given YYYY-MM month."""
        with self._lock:
            row = self._conn.execute(
                "SELECT count FROM ai_usage WHERE user_id=? AND month=?", (user_id, month)
            ).fetchone()
        return int(row["count"]) if row else 0

    def increment_ai_usage(self, user_id: str, month: str) -> int:
        """Increment AI usage count for user in given month. Returns new count."""
        with self._lock:
            self._conn.execute(
